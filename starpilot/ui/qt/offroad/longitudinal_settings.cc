...
StarPilotLongitudinalPanel::StarPilotLongitudinalPanel(StarPilotSettingsWindow *parent, bool forceOpen) : StarPilotListWidget(parent), parent(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> longitudinalToggles {
    ...
    {"NrdrModelLeadTrajectory", tr("Model Lead Trajectory"), tr("<b>Experimental. Plan against the driving model's predicted lead path</b> instead of extrapolating the lead's current acceleration forward. Can brake earlier for a lead that keeps slowing, and react less to brief lead braking. <b>Civic Bosch only.</b>"), ""},
