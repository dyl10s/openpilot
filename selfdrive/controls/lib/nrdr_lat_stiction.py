...
class LatStiction:
  ...
  DES_RATE_MOVE = 2.0         # deg/s; plan actively turning -> MOVE
  DES_RATE_QUIET = 0.8        # deg/s; plan quiet enough to allow HOLD
  WHEEL_RATE_QUIET = 3.0      # deg/s; wheel settled before parking it
  DRIFT_BUDGET_DEGS = 0.35    # deg*s of accumulated held error -> MOVE
  DWELL_S = 0.15              # HOLD-entry conditions must persist this long
  MIN_MOVE_S = 0.25           # once moving, commit to avoid chatter
  MIN_HOLD_S = 0.20           # once holding, commit unless error is urgent
  XFADE_S = 0.08              # bumpless output transition time
  KI_HOLD = 0.10              # torque/(deg*s); micro-integrator while holding
  HOLD_CAPTURE_TAU = 0.30     # s; low-pass used to capture hold torque
  MIN_SPEED = 3.0             # m/s; bypass at parking speeds
  ...
  def update(self, active: bool, v_ego: float, error_deg: float, des_rate_degs: float,
    ...
    """Return the final torque after applying the HOLD/MOVE state machine."""
